...
class L76X(object):
    ...
    def __init__(self,uartx=_uart0, _baudrate = 9600 ):
        if uartx==self._uart1:
            ...
            self.ser = busio.UART(board.GP4,board.GP5, baudrate=_baudrate)
        else:
            #self.ser = UART(uartx,baudrate=_baudrate,tx=Pin(0),rx=Pin(1))
            self.ser = busio.UART(board.GP0,board.GP1, baudrate=_baudrate)

        # self.StandBy = Pin(self.STANDBY_PIN,Pin.OUT)
        # self.Force = Pin(self.FORCE_PIN,Pin.IN)
        # self.StandBy.value(0)
        # self.Force.value(0)
        self.StandBy = digitalio.DigitalInOut(board.GP17)
        self.StandBy.direction = digitalio.Direction.OUTPUT
        self.StandBy.value = 0
        
        self.Force = digitalio.DigitalInOut(board.GP14)
        self.Force.direction = digitalio.Direction.INPUT
        #self.Force.value = 1
...
#         return gcj02_to_bd09(lon, lat)
    ...
    def l76x_set_baudrate(self, _baudrate, uartx=_uart0):
        if self._uart1==uartx:
            # self.ser = UART(uartx,baudrate=_baudrate,tx=Pin(4),rx=Pin(5))
            self.ser = busio.UART(board.GP4,board.GP5, baurdrate=_baudrate)
        else:
            # self.ser = UART(uartx,baudrate=_baudrate,tx=Pin(0),rx=Pin(1))
            self.ser = busio.UART(board.GP0,board.GP1, baurdrate=_baudrate)
    ...
    def l76x_exit_backup_mode(self):
        ...
        # self.Force = Pin(self.FORCE_PIN,Pin.IN)
        #self.Force = digitalio.DigitalInOut(board.GP14)
        #self.Force.direction = digitalio.Direction.OUTPUT
