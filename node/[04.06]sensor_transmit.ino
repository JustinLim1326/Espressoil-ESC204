...
void loop()
{
    ...
    { // Soil moisture sensor reading every 3000 milliseconds
        ...
        if (h <= 15)
        {
            message += "Too arid!;Add shade!/";
            setColor(1, 0, 1, 0, 0, 255); // Purple
            delay(1500);
        }
        if (moist <= 70 and moist >= 30 and t < 27 and t > 23 and h < 50 and h > 20)
        {
            message = "OK/";
        ...
        Serial.println(message);
        String data = "M: " + String(moist) + " H: " + String(h) + " T: " + String(t) + " Hic: " + String(hic) + " GPS: " + GPSData;
        Serial.println(data);
    }
    driver.send((uint8_t *)message.c_str(), message.length());
